...
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	...
	if(on) {
		...
		usleep_range(5000, 5100); /* T1 means iovdd enbaled before avdd*/
		if (avdd_gpio > 0) {
...
power_off:
	...
	/* RST→ [≥1ms] →AVDD/3.3V→ [≥0ms]→DVDD/1.8V */
	...
	usleep_range(5000, 5100); /*Ensure power-off sequence avdd after iovdd*/
	if (iovdd_gpio > 0)
...
static int brl_irq_enbale(struct goodix_ts_core *cd, bool enable)
{
	...
	ts_warn("warnning: irq deepth inbalance!");
	return 0;
}
...
static int brl_fod_attn(struct goodix_ts_core *cd, int value)
{
	struct goodix_ts_cmd cmd;

	cmd.cmd = GOODIX_FOD_ATTN;
	cmd.len = 5;

	if (value == 0) {
		//00 00 05 D2 00 D7 00：进入测试状态，拉低INT2，1s没收到命令后退出测试状态，拉高INT2
		cmd.data[0] = 0X00;
	} else if (value == 1) {
		//00 00 05 D2 01 D8 00：进入测试状态，拉高INT2，1s没收到命令后退出测试状态，拉高INT2
		cmd.data[0] = 0X01;
	} else if (value == 2) {
		//00 00 05 D2 02 D9 00：恢复日常状态，拉高INT2，手指放到FOD区域拉低INT2
		cmd.data[0] = 0X02;
	} else if (value == 3) {
		//00 00 05 D2 03 DA 00：当次上电/RST复位一直拉低INT2，实现永久拉低需要在brl_reset函数每次复位100ms后下发该命令
		cmd.data[0] = 0X03;
	} else if (value == 4) {
		//00 00 05 D2 04 DB 00：亮屏时拉高INT2
		cmd.data[0] = 0X04;
	} else if (value == 5) {
		//00 00 05 D2 05 DC 00：亮屏时拉低INT2
		cmd.data[0] = 0X05;
	} else {
		//error
	...
	if (cd->hw_ops->send_cmd(cd, &cmd)) {
		ts_err("failed send fod_attn cmd, value = %d", value);
		return -EINVAL;
	} else {
		ts_info("fod_attn mode %d", value);
	}
...
static int brl_game(struct goodix_ts_core *cd, u8 data0, u8 data1, bool on)
{
	...
	//touch_irq_boost_switch(0, cd->gamemode_enable);
	return 0;
}
#endif
...
/*
	O10之后使用新协议上报
	enable thp frame data report THP4.0
	enable 00 00 06 90 02 01 99 00
	unable 00 00 06 90 02 00 98 00
*/
...
static struct goodix_ic_info *get_goodix_ic_info(void)
{
	struct goodix_ic_info *ic_info;
	if (!goodix_core_data)
		return NULL;
...
{
	struct goodix_ic_info *ic_info = get_goodix_ic_info();
	if (!ic_info)
		return -1;

	return ic_info->parm.sen_num;
}
...
int goodix_get_freq_num(void)
{
	...
	return ic_info->parm.mutual_freq_num;
}
 ...
 *   If the default interrupt frequency is 135Hz,
 *     index 0x00/0x01/0x02/0x03 corresponds to 135Hz/240Hz/60Hz/180Hz/120Hz.
 * return: 0 on success, otherwise return < 0.
 *
 * NOTE:P11  0x01  corresponds to 135Hz
 */
int goodix_htc_set_active_scan_rate(int scan_rate)
{
	...
	tmp_cmd.len = 5;
	ret = hw_ops->send_cmd(goodix_core_data, &tmp_cmd);
	ts_debug("%s send scan rate index %d, ret %d", ret ? "failed" : "success", index, ret);
...
int goodix_htc_set_scan_freq(u8 index)
{
	...
	if (hw_ops->send_cmd)
		ret = hw_ops->send_cmd(goodix_core_data, &tmp_cmd);
	ts_info("%s send scan freq index %d, ret %d", ret ? "failed" : "success", index, ret);
...
/*
 * Warsaw 新增显示FPS 165HZ，Goodix需要设置FPS到FW,120/144/165
 * value: FPS value
 * FPS < 125                -----> value = 120
 * FPS > 125 && FPS <= 149  -----> value = 144
 * FPS > 149 && FPS <= 170 -----> value = 165
 * others                   -----> value = 120
 * return: 0 on success, otherwise return < 0.
 */
...
int goodix_htc_set_double_scan(int index)
{
	...
	if (hw_ops->send_cmd)
		ret = hw_ops->send_cmd(goodix_core_data, &tmp_cmd);
	ts_info("%s send double scan index %x, ret %d", ret ? "failed" : "success", index, ret);
...
int goodix_htc_set_normalize_study(int *index)
{
	...
	if (hw_ops->send_cmd)
		ret = hw_ops->send_cmd(goodix_core_data, &tmp_cmd);
	ts_info("%s send normalize study index %x %x, ret %d", ret ? "failed" : "success", index[0], index[1], ret);
...
int goodix_htc_set_gesture_feedback(int  index)
{
	...
	if (hw_ops->send_cmd)
		ret = hw_ops->send_cmd(goodix_core_data, &tmp_cmd);
	ts_info("%s send gesture feedback index %d, ret %d", ret ? "failed" : "success", index, ret);
...
int goodix_htc_start_calibration(void)
{
	...
	if (hw_ops->send_cmd)
		ret = hw_ops->send_cmd(goodix_core_data, &tmp_cmd);
	ts_info("%s send freq scan/calibration cmd, ret %d", ret ? "failed" : "success", ret);
	return ret;
}

/*
 * set afe enter or exit glove status
 *
 * data[0]: set true to enter glove, false to exit idle.
 * data[1-2]: down thd, data[3-4]: up thd
 *
 * return: 0 on success, otherwise return < 0.
 */
...
{
	struct goodix_ts_hw_ops *hw_ops;
	struct goodix_ts_cmd glove_cmd;
	int ret = -1;
	ts_info("glove enable: %d, down_thd: %d, up_thd: %d", value[0], value[1], value[2]);

	if (!goodix_core_data)
		return -EINVAL;
	hw_ops = goodix_core_data->hw_ops;
	glove_cmd.cmd = GOODIX_GLOVE_CMD;
	glove_cmd.len = 9;
	glove_cmd.data[0] = value[0] & 0xff;
	glove_cmd.data[1] = value[1] & 0xff;
	glove_cmd.data[2] = (value[1] >> 8) & 0xff;
	glove_cmd.data[3] = value[2] & 0xff;
	glove_cmd.data[4] = (value[2] >> 8) & 0xff;

	if (hw_ops->send_cmd)
		ret = hw_ops->send_cmd(goodix_core_data, &glove_cmd);
	ts_info("%s send glove cmd %d, ret %d", ret ? "failed" : "success", glove_cmd.data[0], ret);
