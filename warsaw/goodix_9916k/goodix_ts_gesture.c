...
int goodix_gesture_ist(struct goodix_ts_core *cd)
{
	...
	case GOODIX_GESTURE_FOD_UP:
		...
		if (!(cd->gesture_enabled & FOD_EN) && (driver_get_touch_mode(TOUCH_ID, Touch_Nonui_Mode) != 2)
...
int gsx_gesture_before_resume(struct goodix_ts_core *cd)
{
	...
	//if (cd->low_attn_status) {
	//	cd->hw_ops->fod_attn(cd, 3);
	//	ts_info("set low forever attn after reset");
	//} else {
	//	if (cd->thp_fod_status == 0 && cd->fod_finger== 0) {
	//		cd->hw_ops->fod_attn(cd, 5);
	//		ts_info("set low attn after reset");
	//	} else {
	//		cd->hw_ops->fod_attn(cd, 4);
