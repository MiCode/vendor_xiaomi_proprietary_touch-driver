...
static ssize_t goodix_ts_double_tap_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int value = driver_get_touch_mode(TOUCH_ID, Touch_Doubletap_Mode);
...
static ssize_t goodix_ts_aod_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int value = driver_get_touch_mode(TOUCH_ID, Touch_Aod_Enable);
	return snprintf(buf, PAGE_SIZE, "state:%s\n",
			value ? "enabled" : "disabled");
}
...
static ssize_t goodix_ts_fod_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int value = driver_get_touch_mode(TOUCH_ID, Touch_Fod_Enable);
	return snprintf(buf, PAGE_SIZE, "state:%s\n",
			value ? "enabled" : "disabled");
}
...
#ifdef TOUCH_FOD_SUPPORT
	if ((goodix_core_data->eventsdata & 0x08) && ((driver_get_touch_mode(TOUCH_ID, Touch_Fod_Enable) == 1) ||
						(driver_get_touch_mode(TOUCH_ID, Touch_Fod_Enable) == 2) ||
						(driver_get_touch_mode(TOUCH_ID, Touch_Fod_Enable) == 3))
			&& (!goodix_core_data->fod_finger)) {
		goodix_core_data->fod_finger = true;
		if (((driver_get_touch_mode(TOUCH_ID, Touch_Fod_Enable) == 3) && (goodix_core_data->super_wallpaper == 0))
...
#ifdef TOUCH_FOD_SUPPORT
			if ((goodix_core_data->eventsdata & 0x08) != 0x08 || !driver_get_touch_mode(TOUCH_ID, Touch_Fod_Enable))
...
static void goodix_ts_release_connects(struct goodix_ts_core *core_data)
{
	...
	if (goodix_core_data->fingerprint_authstate == 0) {
		update_fod_press_status(0);
		ts_debug("fingerprint_authstate: %d update_fod_press_status \n",
...
static void goodix_thp_signal_work(struct work_struct *work)
{
...
#ifdef CONFIG_TOUCH_FACTORY_BUILD
	{
		...
		add_common_data_to_buf(0, SET_CUR_VALUE, Touch_Fod_Enable, 1, &fod_en);
	}
#endif
}
#endif
	...
	if (!strncmp("short", type, 5) || !strncmp("open", type, 4)) {
		...
		if (retval != GTP_RESULT_PASS) {
			if (!strncmp("open", type, 4))
				xiaomi_touch_mievent_report_int(TOUCH_EVENT_OPENTEST_FAIL, 0, "TpOpenTestFail", "goodix", retval);
			if (!strncmp("short", type, 5))
				xiaomi_touch_mievent_report_int(TOUCH_EVENT_SHORTTEST_FAIL, 0, "TpShortTestFail", "goodix", retval);
		}
#endif
...
void goodix_ic_switch_mode(u8 _gesture_type)
{
	...
	ts_debug("fod is 0x%x", driver_get_touch_mode(TOUCH_ID, Touch_Fod_Enable));
...
void goodix_game_mode_update(long mode_update_flag, int mode_value[Touch_Mode_NUM])
{
	...
	if (goodix_core_data->enable_touch_raw) {
		...
		// temp_value = driver_get_touch_mode(TOUCH_ID, Touch_Game_Mode);
...
static int goodix_set_thermal_temp(int temp, bool force)
{
	...
	if (force) {
		...
		add_common_data_to_buf(0, SET_CUR_VALUE, THP_TEMPERATURE_STATUS, 1, &temp);
	}
	ts_info("temp: %d", temp);
...
static void goodix_touch_dfs_test(int value){
	...
	switch(value){
		case TOUCH_EVENT_TRANSFER_ERR:
			xiaomi_touch_mievent_report_str(TOUCH_EVENT_TRANSFER_ERR, 0, "TpTransferErr", "goodix");
			break;
...
upgrade:
    ...
    */ //n11u driver no longer need to send K-matrix to fw
	...
	xiaomi_register_panel_notifier(cd->bus->dev, TOUCH_ID, PANEL_EVENT_NOTIFICATION_PRIMARY,
...
static void goodix_ts_shutdown(struct platform_device *pdev)
{
	...
	xiaomi_unregister_panel_notifier(core_data->bus->dev, TOUCH_ID);
