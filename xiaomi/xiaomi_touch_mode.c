...
static void xiaomi_touch_set_mode_value(common_data_t *common_data)
{
	...
	if (mode != DATA_MODE_144 && mode != DATA_MODE_160)
	...
	case DATA_MODE_19:
		...
		LOG_INFO("DATA_MODE_19 value [%d]", val);
		if (val) {
#if defined(TOUCH_IS_TDDI)
...
int xiaomi_touch_mode(private_data_t *client_private_data, u32 user_size, unsigned long arg)
{
	...
	if (user_size != sizeof(common_data_t)) {
		...
		if (common_data.cmd == SET_CUR_VALUE && common_data.mode == DATA_MODE_144) {
			if (common_data.data_buf[0] != DATA_MODE_144) {
			//	DATA_MODE_144 = common_data.data_buf[0];
		...
		case SET_CMD_FOR_THP:
			...
			return 0;
		case SET_CUR_VALUE:
			if(common_data.mode == DATA_MODE_166 && common_data.data_buf[0] > 100 && common_data.data_buf[0] < 127) {
		...
		if ((common_data.mode > DATA_MODE_19 && common_data.mode < DATA_MODE_41) || common_data.mode == DATA_MODE_57) {
