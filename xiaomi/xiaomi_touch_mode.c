...
u8 xiaomi_get_gesture_type(s8 touch_id)
{
	...
	if (touch_mode[touch_id][TOUCH_SINGLETAP_MODE][GET_CUR_VALUE] &&
		!touch_mode[touch_id][Touch_Nonui_Mode][GET_CUR_VALUE])
...
static bool isfirstscreenOn = true;
#endif
...
static void xiaomi_touch_set_mode_value(common_data_t *common_data)
{
...
#if defined(DISABLE_POWER_STATUS)
		LOG_INFO("Disable Touch_Power_Status value [%d]", val);
		break;
#endif
...
#ifdef TOUCH_SENSORHUB_SUPPORT
#if defined(TOUCH_PLATFORM_XRING)
		tx_msg = xiaomi_get_gesture_type(touch_id);
		ret = sh_ipc_send_with_resp_msg(IPC_VC_AO_NS_SH_AP_TP, TAG_SH_CONNECTIVITY, \
					SYNA_IPC_GESTURE_TYPE, &tx_msg, (u_int16_t)sizeof(tx_msg), &rx_msg, sizeof(rx_msg));
		resp = *(int *)(((struct ipc_pkt*)rx_msg)->data);
		if (ret < 0)
			LOG_ERROR("sh_ipc_send_with_resp_msg gesture_type fail");
#endif
		value = (int)xiaomi_get_gesture_type(touch_id);
		add_common_data_to_buf(0, SET_CUR_VALUE, Touch_Gesture_Type, 1, &value);
#endif
	...
	case TOUCH_FINGERPRINT_AUTHSTATE:
		LOG_INFO("TOUCH_FINGERPRINT_AUTHSTATE: %d", val);
		if (xiaomi_touch_driver_param->hardware_operation.set_mode_value)
			xiaomi_touch_driver_param->hardware_operation.set_mode_value(mode, &val);
		break;
	default:
